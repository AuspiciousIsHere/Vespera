...
import { loginNotifEmail, sendResetPasswordEmail, sendWelcomeEmail } from "../utils/sendEmail.ts";
...
export const signup = catchAsync(async (req, res, next) => {
  ...
  await sendWelcomeEmail(newUser, "localhost:5173");
...
export const login = catchAsync(async (req, res, next) => {
  ...
  const loginTime = new Date().toLocaleString("en-US", { timeZone: "UTC", dateStyle: "full", timeStyle: "short" });

  loginNotifEmail(user, loginTime, deviceInfo);
...
export const protect = catchAsync(async (req, res, next) => {
  ...
  if (token === undefined || token.length === 0) return next(new AppError("You are not logged in! please login to get access", 401));
...
export const updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.currentPassword, user.password))) return next(new AppError("Your current password is wrong!", 400));

  if (req.body.newPassword !== req.body.confirmPassword) return next(new AppError("Password does not match!", 400));
